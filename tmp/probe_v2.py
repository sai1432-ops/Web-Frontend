...
# Test Step 2 endpoints based on common Flask patterns and project naming
endpoints = [
    ...
    "/user/distribution-confirm",
    "/user/receipt-confirm",
    "/user/confirm-receipt",
    "/api/user/confirm-kit-receipt",
    "/api/user/confirm-distribution",
    "/user/confirm-registration" # sometimes misused
]

for p in endpoints:
    status, body = req(p, {"user_id": 1, "kit_unique_id": "test", "dealer_qr_value": "test"})
